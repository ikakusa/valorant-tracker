import requests
# agent list toka wo nantoka site motimoti suru yotei de aru woawhahahahahah
weaponID_maps = [
    { "odin": "63e6c2b6-4a8e-869c-3d4c-e38355226584" },
    { "ares": "55d8a0f4-4274-ca67-fe2c-06ab45efdf58" },
    { "vandal": "9c82e19d-4575-0200-1a81-3eacf00cf872" },
    { "bulldog": "ae3de142-4d85-2547-dd26-4e90bed35cf7" },
    { "phantom": "ee8e8d15-496b-07ac-e5f6-8fae5d4c7b1a" },
    { "judge": "ec845bf4-4f79-ddda-a3da-0db3774b2794" },
    { "bucky": "910be174-449b-c412-ab22-d0873436b21b" },
    { "frenzy": "44d4e95c-4157-0037-81b2-17841bf2e8e3" },
    { "classic": "29a0cfab-485b-f5d5-779a-b59f85e204a8" },
    { "ghost": "1baa85b4-4c70-1284-64bb-6481dfc3bb4e" },
    { "sheriff": "e336c6b8-418d-9340-d77f-7a9e4cfe0702" },
    { "shorty": "42da8ccc-40d5-affc-beec-15aa47b42eda" },
    { "operator": "a03b24d3-4319-996d-0f8c-94bbfba1dfc7" },
    { "guardian": "4ade7faa-4cf1-8376-95ef-39884480959b" },
    { "outlaw": "5f0aaf7a-4289-3998-d5ff-eb9a5cf7ef5c" },
    { "marshal": "c4883e50-4494-202c-3ec3-6b8a9284f00b" },
    { "spectre": "462080d1-4035-2937-7c09-27aa2a5c27a7" },
    { "stinger": "f7e1b454-4ad4-1063-ec0a-159e56b58941" },
    { "melee": "2f59173c-4bed-b6c3-2191-dea9b58be9c7" }
]

weapon_data = { weapon_data["uuid"]: weapon_data for weapon_data in requests.get("https://valorant-api.com/v1/weapons").json()["data"] }
agent_data = { agent_data["uuid"]: agent_data for agent_data in requests.get("https://valorant-api.com/v1/agents").json()["data"] }
#上のやつに新エージェントを追加する
if not (agent_data.get("92eeef5d-43b5-1d4a-8d03-b3927a09034b")):
    agent_data.setdefault("92eeef5d-43b5-1d4a-8d03-b3927a09034b", {}).update(
        {
            "displayName": "Veto",
            "displayIcon": "https://media.valorant-api.com/competitivetiers/564d8e28-c226-3180-6285-e48a390db8b1/0/smallicon.png"
        }
    )
season_data = { season_data["uuid"]: season_data for season_data in requests.get("https://valorant-api.com/v1/seasons").json()["data"] }
competitive_tiers_data = sorted(requests.get("https://valorant-api.com/v1/competitiveTiers").json()["data"][-1]["tiers"], key=lambda x: x["tier"])