...
class UnofficialAPI:
    ...
    def get_competitive_status(self, puuid, start = 0, end = 10, queue="competitive"):
        header = self.base_pvp_header.copy()
        return self.handle_pvp_request(
            suffix=f"mmr/v1/players/{puuid}/competitiveupdates?startIndex={start}&endIndex={end}&queue={queue}", header=header
        ).get_json()
    ...
    def get_game_state(self, presences):
        try:
            return self.get_private_presence(presences)["matchPresenceData"]["sessionLoopState"]
    ...
    def get_player_party(self, puuid):
        header = self.base_pvp_header.copy()

        header["X-Riot-ClientVersion"] = self.client_version
        return self.handle_pvp_request(
            f"parties/v1/players/{puuid}",
            prefix=f"glz-{self.region}-1",
            header=header,
        ).get_json()
    ...
    PROFILE
    """
    def get_my_playercard(self):
        priv = self.get_private_presence(self.get_presence())
        cardID = priv.get("playerCardId")
        if cardID is not None:
            res = req.get(f"https://valorant-api.com/v1/playercards/{cardID}").json()["data"]
            return res
